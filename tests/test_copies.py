"""Die aus den Vorgängern kopierten Bausteine (Zufallsgenerator, Flusskerne) sind bewacht: dieselben Zahlen wie in edmonds-karp-demo, dinic-demo und graph-cuts-demo."""

import gh_bk as bk
import gh_dinic as dn
...
def test_flow_kernels_reproduce_the_predecessor_numbers():
    """dinic-demo, Preset "Zuordnung als Fluss": eine Phase (Niveau 3) mit 5 Wegen, 69 durchsuchte Kanten gegen 100 bei Edmonds-Karp; Boykov-Kolmogorov findet denselben Wert."""
    net = _assignment()
    d = dn.dinic(net)
    ...
    assert bk.bk_maxflow(net).value == 5
